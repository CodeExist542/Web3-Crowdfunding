...
class UserService {
    ...
    async loginUser(body) {
        try {
            ...
            const user = await userModel_1.UserModel.findOne({ Email: body.Email });
            if (!user) {
    ...
    async connectWallet(body) {
        try {
            ...
            if (user.WalletAddress && user.WalletAddress !== body.WalletAddress) {
                throw new Error("Anda tidak bisa mengganti wallet yang sudah terhubung.");
            }
            ...
            if (existingUser && existingUser.Email !== user.Email) {
                throw new Error("Wallet ini sudah pernah terhubung ke akun lain dan tidak bisa digunakan lagi.");
            }
